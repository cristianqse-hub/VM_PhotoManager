...
class ComposeObject:
    ...
    output_name: str = ""
    align_pattern: str = ""
    align_pattern_threshold: float = 0.6
    align_pattern_count: int = 4
    slope: float = 0.0
...
def apply_params(obj: ComposeObject, params: dict[str, str]) -> None:
    for key, value in params.items():
        ...
        elif key_upper == "ALIGN_PATTERN_THRESHOLD":
            try:
                obj.align_pattern_threshold = float(value)
            except ValueError:
                print(f"[WRN] ALIGN_PATTERN_THRESHOLD invalido para ID={obj.object_id}: {value}")
        elif key_upper == "ALIGN_PATTERN_COUNT":
            try:
                parsed_count = int(value)
                obj.align_pattern_count = max(1, parsed_count)
            except ValueError:
                print(f"[WRN] ALIGN_PATTERN_COUNT invalido para ID={obj.object_id}: {value}")
    ...
    try:
        ...
        if obj.align_pattern.strip():
            ...
            if pattern_path.is_file():
                ...
                if pattern is None:
                    print(f"[WRN] No se pudo leer align_pattern para ID={obj.object_id}: {pattern_path}")
                else:
                    try:
                        matches = find_pattern_matches(
                            image=pano,
                            pattern=pattern,
                            threshold=obj.align_pattern_threshold,
                            expected_count=obj.align_pattern_count,
                        )
                        print(
                            "[INF] Alineacion por patron "
                            f"ID={obj.object_id}: encontrados={len(matches)} "
                            f"(count_cfg={obj.align_pattern_count}, threshold={obj.align_pattern_threshold:.3f})"
                        )
                        for idx, (score, center, _) in enumerate(matches, start=1):
                            print(
                                f"[INF]    {idx}. center=({center[0]:.1f}, {center[1]:.1f}) "
                                f"score={score:.3f}"
                            )

                        if len(matches) >= 2:
                            centers = np.array([m[1] for m in matches], dtype=np.float64)
                            x_vals = centers[:, 0]
                            y_vals = centers[:, 1]
                            if np.ptp(x_vals) > 1e-9:
                                slope, intercept = np.polyfit(x_vals, y_vals, deg=1)
                                angle_deg = -float(np.degrees(np.arctan(slope)))
                                print(
                                    "[INF] Ajuste lineal por minimos cuadrados "
                                    f"ID={obj.object_id}: slope={float(slope):.6f}, "
                                    f"intercept={float(intercept):.3f}, angle={angle_deg:.4f}"
                                )
                                ...
                                best_angle_txt, best_img = min(
                                    candidates, key=lambda item: item[1].shape[0]
                                )
                                pano = best_img
                                ...
                                print(
                                    f"[WRN] Rotacion por patron omitida ID={obj.object_id}: "
                                    "dispersion en X insuficiente para ajuste lineal."
                                )
                        else:
                            print(
                                f"[WRN] Rotacion por patron omitida ID={obj.object_id}: "
                                f"coincidencias insuficientes ({len(matches)})."
                            )
                    except Exception as align_exc:
                        print(
                            f"[WRN] Fallo en correccion por patron ID={obj.object_id}: {align_exc}. "
                            "Se genera sin correccion por patron."
                        )
            else:
